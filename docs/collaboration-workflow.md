...
- https://github.com/Muru24/Unreal-SlaytheChampions/issues/1
- https://github.com/Muru24/Unreal-SlaytheChampions/issues/2
- https://github.com/Muru24/Unreal-SlaytheChampions/issues/3
...
- GitHub 라벨은 원격 생성 완료.
- GitHub Phase 1 Project 보드는 생성 완료.
- Phase 1 초기 이슈 3개는 Project에 등록 완료.
- Notion 작업 카드에는 관련 GitHub Issue URL을 수동 연결 완료.

## 자동 동기화 여부

현재는 자동 동기화를 사용하지 않는다.

자동으로 처리되지 않는 항목:

- GitHub Issue 생성 시 Notion 작업 카드 자동 생성
- GitHub Issue 상태 변경 시 Notion 상태 자동 변경
- Notion 작업 상태 변경 시 GitHub Project 상태 자동 변경

수동 운영 기준:

1. Notion에서 작업을 만든다.
2. 구현이 필요하면 GitHub Issue를 만든다.
3. Notion 작업 카드의 `GitHub 링크`에 Issue URL을 붙인다.
4. GitHub Project에서 개발 진행을 관리한다.
5. 완료 시 Notion 상태도 직접 `완료`로 변경한다.
